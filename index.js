...
// ===== LIVE DATA (COINBASE = NO BLOCKING) =====
...
function buildAlert(btc, wti, etf) {
  ...
  const structure = "lower high forming";
  const sfp = "weekly SFP forming";
  const oilState = wti > 95 ? "HEADWIND" : "TAILWIND";

  const reason = `ETF bid cushioning downside, but crypto lagging beta + Parrot thesis active while BTC trades below resistance; no meaningful div; NEXT bull trigger 78.33k, NEXT bear trigger 76.93k, BW:1/3`;
